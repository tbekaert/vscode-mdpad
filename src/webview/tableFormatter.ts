...
const buildRow = (cells: string[], widths: number[], isSep: boolean): string => {
  const parts = cells.map((cell, i) => {
    ...
    if (isSep) {
      ...
      const inner = '-'.repeat(Math.max(w, 3) - (left ? 1 : 0) - (right ? 1 : 0));
...
const mapCursorPosition = (
    ...
    if (charCount + oldLines[i].length >= cursorOffset) {
      ...
      for (let j = 0; j < cells.length; j++) {
        ...
        if (posInLine <= cellEnd) {
          cursorCol = j;
          cursorInCell = Math.min(posInLine - cellStart, cells[j].trimEnd().length);
          break;
        }
        cellStart = cellEnd + 1;
      }
  ...
  if (cursorRow < newLines.length) {
    ...
    newCursorPos += Math.min(newCellStart + cursorInCell, newLines[cursorRow].length);
  }
...
export const tableAutoFormat = ViewPlugin.fromClass(
  class {
    ...
    update(update: ViewUpdate): void {
      ...
      tree.iterate({
        enter(node) {
          if (node.name === 'Table' && node.from <= cursor && node.to >= cursor) {
            tableFrom = node.from;
            tableTo = node.to;
          }
        },
    ...
    private doFormat(view: EditorView): void {
      ...
      const newCursorPos = mapCursorPosition(tableText, formatted, cursorOffset, currentFrom);

      this.isFormatting = true;
      view.dispatch({
        changes: { from: currentFrom, to: currentTo, insert: formatted },
        selection: { anchor: Math.min(newCursorPos, currentFrom + formatted.length) },
      });
